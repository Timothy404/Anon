		...
		if (!isset($_POST['sendGroupName'])){
			echo "<button class='btn btn-outline-danger btn-sm' onclick='window.location.assign('" . PROOT . "')' style='margin-right: 5px; float: right;'> < Back</button>Select A Group";
		}else{
		?>
		...
		<div class="tab-content" id="pills-tabContent">
  		...
  		<input type="text" class="form-control" name="newGroupName" placeholder="Enter New Group Name" value="<?php echo ucwords($GroupName); ?>" required>
		...
		// use array to pull them out
		if ($group_contacts != null) {
				foreach ($group_contacts as $value) {
					$foreign_key = $value['cnt_id'];
					$sql = $activities->select("stored_contacts", "WHERE `id` = $foreign_key");
						if ($sql != null) {
							foreach ($sql as $primary_key) {
				?>
					<tr>
		<td><?php echo $primary_key['cnt_title'] . " " . $primary_key['cnt_firstname'] . " " . $primary_key['cnt_lastname']; ?></td>
		<td><?php echo $primary_key['cnt_email']; ?></td>
		<td>
		<form method="POST" action="<?=PROOT?>ContactGroups">
		<input value="<?php echo $primary_key['id']; ?>" name="DELid" hidden>
		<input hidden name="sendGroupName" value="<?php echo $GroupName; ?>">
		<button class="btn btn-outline-danger btn-sm" name="DelUserBTN">Delete</button>
			...
			// $modify = $foreign_key . "y";
			// $split = explode("y", $modify);
			// $cleanarray = array_diff($split, ['']);
			// print_r(array_merge($cleanarray));
		}
		$missing_id = "" ;
		// $activities->delete($simplifiedName, "WHERE `id` = ");
		}
		}else{ 
