<?php
	...
	if (isset($_POST['sendGroupName'])) {
		...
		$activities->delete("$simplify", "WHERE `cnt_id` = $id ");
		...
		$checksql = "SELECT * FROM `$simple_table` WHERE `cnt_id` = '$id' ";
        $result = $activities->connect()->query($checksql);
        $resultCheck = $result->num_rows;
		...
		$activities->insert("$simple_table", "`cnt_id`, `cnt_fullname`, `cnt_email`", "'$id', '', ''");
