...
$success2 = "";
  ...
  $DataBConn = $DB->connect();
   ...
   if (empty($title) || empty($firstname) || empty($lastname) || empty($gender) || empty($address) || empty($email) || empty($number)){
    ...
    if ($NumRows > 0) {          
      echo "<script>alert('ERROR: A Contact Exist with the same EMAIL or NUMBER')</script>";
      $error2 = "<h6 class='text-danger'><i class='icon-info'></i> ERROR: A Contact Exist with the same EMAIL or NUMBER</h6>";
    }else{
      $value = "'$firstname', '$email', '$address', '$title', '$number', '$gender', '$lastname', '$day', '$month', '$year', '$user_bg_value', '', '' ";
      $saveUser = $activities->insert("stored_contacts", "`cnt_firstname`, `cnt_email`, `cnt_address`, `cnt_title`, `cnt_number`, `cnt_gender`, `cnt_lastname`, `cnt_day`, `cnt_month`, `cnt_year`, `bg1`, `cnt_CreatedBy`, `LastUpdatedBy` ", $value);
        $success1 = "<script>" . "alert('SUCCESS: $title $firstname $lastname Hass been Added Successfully')" . "</script>";
        $success2 = "<p class='text-success'> $title $firstname $lastname Has been Added Successfully </p>";
  }  
  }
  }
